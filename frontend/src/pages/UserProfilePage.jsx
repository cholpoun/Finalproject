    ...
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        setMessage(
          <span>
            It seems like you need to log in again due to session expiration.
            Please <Link to="/users/authenticate">log in again</Link> to continue.
          </span>
        );
      } finally {
        setLoading(false);
        ...
        {user.purchasedTickets.length === 0 ? (
          ...
          user.purchasedTickets.map((ticket, index) => (
            <TicketCard key={index}>
              ...
              <p><strong>Total Price:</strong> {ticket.totalPrice} SEK</p>
              <QRCodeContainer>
                <QRCode value={String(ticket.festivalName + " " + ticket.quantity)} size={100} />
              </QRCodeContainer>
            </TicketCard>
          ))
        )}
